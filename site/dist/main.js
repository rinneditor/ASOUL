...
const elements = {
    summaryCards: queryElement("#summary-cards"),
    spotlightGrid: queryElement("#spotlight-grid"),
    memberFilters: queryElement("#member-filters"),
    yearFilters: queryElement("#year-filters"),
    searchInput: queryElement("#search-input"),
    sortSelect: queryElement("#sort-select"),
    resultsMeta: queryElement("#results-meta"),
    galleryGrid: queryElement("#gallery-grid"),
    cardTemplate: queryElement("#card-template"),
    modal: queryElement("#detail-modal"),
    modalClose: queryElement("#detail-close"),
    detailImage: queryElement("#detail-image"),
    detailTitle: queryElement("#detail-title"),
    detailSubtitle: queryElement("#detail-subtitle"),
    detailBadges: queryElement("#detail-badges"),
    detailGrid: queryElement("#detail-grid"),
    detailFileLink: queryElement("#detail-file-link"),
    detailSourceLink: queryElement("#detail-source-link"),
};
void init();
async function init() {
    bindEvents();
...
function collectYears(assets) {
    const unique = new Set(assets.map((asset) => asset._year).filter((year) => Number.isInteger(year)));
...
function renderYearFilters() {
    ...
    state.years.forEach((year) => {
        buttons.push(`<button class="chip" data-year="${year}" type="button">${year}</button>`);
    });
...
function renderSpotlights() {
    const members = ["Bella", "Diana", "Eileen"];
    ...
    elements.spotlightGrid.innerHTML = spotlights
        ...
        <article class="spotlight-card" style="--accent-soft: ${asset._accentSoft}">
          ...
          <div>
            ...
            <p class="spotlight-card__meta">${escapeHtml(`${asset._subtitle} · ${asset.width}×${asset.height}`)}</p>
          </div>
        </article>
      `)
        .join("");
...
function renderGallery() {
    ...
    state.filteredAssets.forEach((asset) => {
        ...
        const mediaButton = queryElement(".outfit-card__media", node);
        const image = queryElement(".outfit-card__image", node);
        const badges = queryElement(".outfit-card__badges", node);
        const title = queryElement(".outfit-card__title", node);
        const meta = queryElement(".outfit-card__meta", node);
...
function renderFatal(message) {
    ...
    elements.galleryGrid.innerHTML = `<div class="gallery-empty">${escapeHtml(message)}<br />请通过本地 HTTP 服务或 GitHub Pages 打开站点，不要直接双击本地文件。</div>`;
}
...
function parseMemberFilter(value) {
